...
// NUEVA MEJORA 5: variables para edición
let editingTransactionId = null;

function initRegistro() {
  // Si venimos de edición, cargar datos
  if (window._editingTransactionId && window.editTransactionData) {
    editingTransactionId = window._editingTransactionId;
    const tx = window.editTransactionData;
    currentType = tx.type;
    currentAmount = tx.amount;
    currentCategory = tx.category;
    setTipoRegistro(currentType);
    document.getElementById('input-monto-custom').value = currentAmount;
    updateAmountDisplay();
    renderCategorias();
    if (currentCategory && !currentCategory.startsWith('otro_libre:')) {
      seleccionarCategoria(currentCategory);
    } else if (currentCategory && currentCategory.startsWith('otro_libre:')) {
      toggleCategoriaOtro();
      document.getElementById('otro-input-text').value = currentCategory.slice('otro_libre:'.length);
    }
    // Limpiar globales
    window._editingTransactionId = null;
    window.editTransactionData = null;
  } else {
    ...
    document.getElementById('monto-display').textContent = `${getCurrencySymbol()} 0.00`;
...
function _renderOtherInput(visible, currentText, modeClass) {
  ...
  const placeholder = isGasto ? '¿En qué gastaste?' : '¿De dónde proviene?';
  const hint = isGasto ? 'Describe brevemente — se guardará como etiqueta' : 'Describe la fuente — se guardará como etiqueta';
  wrap.innerHTML = `
    <div class="otro-input-container ${modeClass}">
      <input type="text" id="otro-input-text" class="input-field otro-input"
        placeholder="${placeholder}" maxlength="30" autocomplete="off"
        value="${currentText}" oninput="onOtroTextChange(this.value)">
      <span class="otro-input-hint">${hint}</span>
    </div>`;
  setTimeout(() => {
...
function _renderCustomCategoryLabel() { /* no longer needed */ }
...
function toggleCategoriaOtro() {
  const isOtherActive = currentCategory === 'otro' || (currentCategory && currentCategory.startsWith('otro_libre:'));
  if (isOtherActive) {
    currentCategory = null;
  } else {
...
function updateAmountDisplay() {
  document.getElementById('monto-display').textContent = `${getCurrencySymbol()} ${currentAmount.toFixed(2)}`;
}
...
function updateBudgetDisplay() {
  ...
  const el = document.getElementById('presupuesto-info');
  if (!el) return;
  const boltSVG = `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M13 3l0 7l6 0l-8 11l0 -7l-6 0l8 -11"/></svg>`;
  if (!budget) {
    el.innerHTML = `<div class="presupuesto-info-icon">${boltSVG}</div><div class="presupuesto-info-row"><span class="presupuesto-info-label">Presupuesto diario</span><span class="presupuesto-info-monto">No configurado</span></div>`;
    return;
  }
  ...
  el.innerHTML = `<div class="presupuesto-info-icon ${levelClass}">${boltSVG}</div><div class="presupuesto-info-row"><span class="presupuesto-info-label">Presupuesto diario</span><span class="presupuesto-info-monto ${levelClass}">${symbol} ${Math.abs(remaining).toFixed(2)} ${remaining < 0 ? 'excedido' : 'restantes'}</span></div>`;
}
...
function showBalanceWarning(balance, currency) {
  ...
  el.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 9v4"/><path d="M10.363 3.591l-8.106 13.534a1.914 1.914 0 0 0 1.636 2.871h16.214a1.914 1.914 0 0 0 1.636-2.87l-8.106-13.536a1.914 1.914 0 0 0-3.274 0"/><path d="M12 16h.01"/></svg><div class="aviso-balance-texto"><span>Monto mayor al balance actual.</span><span>Balance: ${currency} ${balance.toFixed(2)}</span></div>`;
...
// ─────────────────────────────────────────────────────────────────
// CONFIRM REGISTRATION (con soporte para edición)
// ─────────────────────────────────────────────────────────────────
function confirmarRegistro() {
  ...
  // Si estamos editando, actualizar transacción existente
  if (editingTransactionId) {
    Storage.updateTransaction(editingTransactionId, {
      type: currentType,
    ...
    // Actualizar racha (si el usuario registra un movimiento, cuenta como actividad)
    Storage.updateStreak(true);
    editingTransactionId = null;
    navigate(SCREENS.ACTIVIDAD);
    return;
  }
  ...
  Storage.updateStreak(true); // Actualizar racha al registrar movimiento
  ...
  Toast.success(`${typeLabel} registrado`, `${symbol}${currentAmount.toFixed(2)} · ${catLabel}`);
...
function checkBudgetAfterExpense() {
  ...
  if (!budget) return;
  const today = new Date();
  const todayExpenses = Storage.getTransactions().filter(t => {
    const d = new Date(t.date);
    return t.type === 'gasto' && d.toDateString() === today.toDateString();
  }).reduce((sum, t) => sum + t.amount, 0);
  ...
  agregarNotificacion(NOTIF_TIPO.DANGER, '¡Presupuesto excedido!', `Superaste tu límite diario de ${symbol}${budget.toFixed(2)}. Llevas ${symbol}${excess} de más hoy.`);
}
...
function _checkGoalImmediately() {
  ...
  const weeklySaved = txs.filter(t => new Date(t.date) >= weekStart).reduce((total, t) => t.type === 'ingreso' ? total + t.amount : total - t.amount, 0);
